/**
 * Force Clear All Calendar Events Script
 *
 * This script directly deletes ALL events from Google Calendar
 * regardless of database tracking. Use this when events exist in
 * Google Calendar but not properly tracked in the database.
 */
...
async function forceClearAllCalendarEvents() {
  console.log('🧹 Force clearing ALL calendar events from Google Calendar...\n');
  ...
  try {
    ...
    if (totalDeleted > 0) {
      console.log('\n🎉 All calendar events have been force cleared!');
      console.log('💡 Your Google Calendar should now be clean for testing');
    }

  } catch (error) {
    console.error('❌ Failed to force clear calendar events:', error);
  }
}
