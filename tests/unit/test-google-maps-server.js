...
async function testPlacesAPI() {
    try {
        // Test the new Places API (Places API (New))
        ...
        if (response.ok) {
            ...
            console.log(`   Found ${data.places?.length || 0} places for "hospital in Dubai"`);
        } else {
            const errorData = await response.json();
            console.log(`❌ Places API (New) error: ${response.status}`);
            if (errorData.error?.message) {
                console.log(`   Error message: ${errorData.error.message}`);
            }
        }
    } catch (error) {
        console.log(`❌ Places API (New) request failed: ${error.message}`);
    }
}
