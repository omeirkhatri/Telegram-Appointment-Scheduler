...
- [ ] 1.0 Database & Types Setup
  - [ ] 1.1 Review and apply the database migration `supabase/migrations/20250225000000_finance_schema.sql`.
  - [ ] 1.2 Create TypeScript definitions for `ServiceContract`, `ContractPayment`, and `StaffPayout` in `src/types/finance.ts`.
  - [ ] 1.3 Update `Appointment` type in `src/types/appointment.ts` to include `service_contract_id`.
  - [ ] 1.4 Verify RLS policies for the new tables to ensure proper access control (admin/manager only).
  - [ ] 1.5 Run database type generation or manually ensure Supabase types and database schema are in sync.

- [ ] 2.0 Backend Services Implementation
  - [ ] 2.1 Create `src/services/financeService.ts` skeleton with error handling and logging.
  - [ ] 2.2 Implement contract management methods: `createContract`, `updateContract`, `getContract`, `listContracts` (with filters for status/patient).
  - [ ] 2.3 Implement payment tracking methods: `recordPayment`, `getPaymentsByContract`, `calculateContractBalance` (Total - Sum(Payments)).
  - [ ] 2.4 Implement staff payout methods: `recordPayout`, `getPayoutsByStaff`, `calculateStaffEarnings` (Completed Appointments * Rate).
  - [ ] 2.5 Implement `linkAppointmentToContract` and `getAppointmentsByContract` to support tracking usage.
  - [ ] 2.6 Add unit tests for `financeService.ts` in `src/services/__tests__/financeService.test.ts` covering calculations and edge cases.

- [ ] 3.0 Finance Dashboard (UI)
  - [ ] 3.1 Create the main finance layout and navigation in `src/app/finance/layout.tsx` (Sidebar/Tabs for Contracts, Payouts).
  - [ ] 3.2 Implement `src/app/finance/page.tsx` to fetch and display a summary of active contracts.
