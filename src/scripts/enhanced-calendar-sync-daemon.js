/**
 * Enhanced Calendar Sync Daemon with Orphaned Event Cleanup
 *
 * This daemon handles:
 ...
 * - Cleaning up orphaned calendar events (events without database records)
 */
...
// NEW: Handle orphaned calendar events (events without database records)
async function handleOrphanedCalendarEvents() {
  ...
  try {
    ...
    console.error('❌ Error handling orphaned calendar events:', error);
  }
}
...
function startDaemon() {
  console.log('🚀 Starting Enhanced Calendar Sync Daemon...');
  console.log('📅 Will sync appointments every 10 seconds');
  console.log('🧹 Will clean up orphaned events automatically');
...
// Stop the daemon
function stopDaemon() {
  console.log('🛑 Stopping Enhanced Calendar Sync Daemon...');
  if (syncInterval) {
