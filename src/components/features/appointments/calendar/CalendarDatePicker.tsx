...
export function CalendarDatePicker({
  ...
  const getButtonText = () => {
    ...
    if (currentView === 'dayGridMonth') {
      ...
      // For week view, show the Monday of the current week
      const mondayOfWeek = new Date(dateToShow);
      const dayOfWeek = mondayOfWeek.getDay();
      const daysToMonday = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // Convert Sunday (0) to 6, others shift by -1
      mondayOfWeek.setDate(mondayOfWeek.getDate() - daysToMonday);

      return mondayOfWeek.toLocaleDateString('en', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
        timeZone: 'Asia/Dubai'
  ...
  const getFirstDayOfMonth = (date: Date) => {
    ...
    return dayOfWeek === 0 ? 6 : dayOfWeek - 1; // Convert Sunday (0) to 6, others shift by -1
  };
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className="p-4">
        ...
        <div className="grid grid-cols-[2rem,1fr,2rem] items-center mb-4">
          <button
            onClick={() => {
