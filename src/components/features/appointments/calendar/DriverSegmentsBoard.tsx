...
export function DriverSegmentsBoard({
  ...
  }, [segments, searchTerm, statusFilter, typeFilter]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            {selectedDate ? `Segments for ${formatDate(selectedDate)}` : 'All driver segments'}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Driver Segments Found</h3>
            <p className="text-gray-600">
              {showConflictsOnly
                ? 'No drivers with conflicts found for the selected criteria.'
                : 'No driver segments found for the selected criteria.'
              }
            </p>
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          {displayTimelines.map((timeline) => (
            <Card key={timeline.driver.id} className="overflow-hidden">
              <CardHeader
                    ...
                    {timeline.segments.map((segment, index) => (
                      <div
                        ...
                        className={`p-4 border rounded-lg hover:shadow-md transition-shadow ${
                          segment.manual_override ? 'border-orange-300 bg-orange-50' : 'border-gray-200'
                        }`}
                        role="article"
                      ...
                      >
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-4 lg:space-y-0">
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              <Badge variant="outline">
                                {getTransportationSegmentTypeLabel(segment.segment_type)}
                              </Badge>
                            ...
                            {/* Segment Conflicts */}
                            {timeline.conflicts.filter(c => c.segmentId === segment.id).length > 0 && (
                              <div className="mt-3 space-y-1">
                                {timeline.conflicts
                                  .filter(c => c.segmentId === segment.id)
                                  .map((conflict, conflictIndex) => (
                                    <div key={conflictIndex} className="flex items-center space-x-2 text-sm text-red-600">
                                      {getConflictIcon(conflict.severity)}
                                      <span className="break-words">{conflict.message}</span>
                                    </div>
                                  ))}
                              </div>
                            )}
                          ...
                          {/* Segment Actions - Responsive Layout */}
                          <div className="flex flex-col sm:flex-row lg:flex-col items-stretch sm:items-center lg:items-stretch space-y-2 sm:space-y-0 sm:space-x-2 lg:space-x-0 lg:space-y-2 lg:ml-4">
                            ...
                            {onSegmentReassign && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleReassignSegment(segment)}
                                className="w-full sm:w-auto"
