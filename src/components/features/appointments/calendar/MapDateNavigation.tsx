...
export function MapDateNavigation({
  ...
  const isSelectedDateToday = isToday(currentDate);
  const todayHighlightClasses = isSelectedDateToday
    ? 'bg-[#ff5c00] text-white rounded-md border border-[#ff5c00] shadow-sm'
    : '';
  const todayLabelClasses = isSelectedDateToday ? 'text-white/80' : 'text-[--primary]';
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-center">
          <div className={`font-medium ${sizeConfig.input.replace('px-3 py-1.5', 'px-2 py-1')} ${todayHighlightClasses}`}>
            {formatDisplayDate(currentDate)}
          </div>
          {isSelectedDateToday && (
            <div className={`text-xs font-medium ${todayLabelClasses}`}>Today</div>
          )}
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`px-3 py-1 text-center min-w-[120px] ${sizeConfig.input} ${todayHighlightClasses}`}>
