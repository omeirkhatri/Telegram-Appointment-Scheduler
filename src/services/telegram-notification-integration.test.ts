...
describe('Telegram Notification Service Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockPatientService = require('@/services/patientService');
  });
  ...
  describe('Same-day appointment notification flow', () => {
    it('should send notifications when creating same-day appointment', async () => {
      ...
      mockStaffService.getStaffMember = jest.fn().mockResolvedValue(mockStaff);
      mockPatientService.getPatient = jest.fn().mockResolvedValue({
        id: 'patient-1',
        name: 'John Doe',
        phone: '+971501234567'
      });
      ...
      // Clear module cache and re-import to ensure mocks are applied
      jest.resetModules();
    ...
    it('should handle notification failures gracefully', async () => {
      ...
      expect(result.results[0].error).toContain('Telegram API error');
    });
  });
  ...
  describe('1-hour reminder notification flow', () => {
    ...
    it('should handle staff without Telegram ID', async () => {
      ...
      expect(result.results).toHaveLength(1);
      expect(result.results[0].success).toBe(false);
      expect(result.results[0].error).toContain('No Telegram user ID');
