  ...
  try {
    ...
    if (staffAssignments.length > 0) {
      ...
      for (const assignment of staffAssignments) {
        try {
          const assigned = await appointmentStaffService.assignStaffToAppointment(
            newAppointment.id,
            [assignment],
          );
      ...
      const sourceStaff = await appointmentStaffService.getStaffForAppointment(appointmentId);
        ...
        try {
          const assigned = await appointmentStaffService.assignStaffToAppointment(
            ...
            [{
              ...
              is_primary: staffMember.is_primary,
            }],
          );
          assignedStaff.push(assigned);
        } catch (error) {
